...
def convert_selected_pages_to_images(pdf_path, output_dir, pages_to_extract):
    ...
    pages_to_extract: list of 1-based page numbers to extract (e.g., [1, 9])
    """
...
def perform_ocr_on_selected_images(image_dir, output_dir, pages_to_ocr):
    ...
    Saves results as cover_page.txt and truth_poem.txt.
    """
    image_dir = Path(image_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    page_to_file = {1: "cover_page.txt", 9: "truth_poem.txt"}

    for page_num in pages_to_ocr:
        image_file = image_dir / f"page_{page_num}.png"
        output_file = output_dir / page_to_file.get(page_num, f"page_{page_num}.txt")
        if image_file.exists():
            print(f"Processing OCR for {image_file}...")
            try:
...
if __name__ == "__main__":
    ...
    pages_to_extract = [1, 9]  # Cover page and poem page
